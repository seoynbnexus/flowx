...
import {
  uuidToBuffer,
  generateUuid,
} from "../../../shared/utils/uuid.utils.js";
import {
  sendSuccess,
  sendError,
...
// Normalize app_config values across local and production environments
// Depending on the database column/driver configuration, config_value may
// be returned as a JSON string or as an already-parsed JavaScript object
  ...
  if (existing) {
    ...
    await query(
      ...
      [
        ...
        "Feature visibility toggles per role (true = visible)",
...
export async function updateFeatureVisibility(req, res, next) {
  try {
    ...
    const persisted = await transaction(async (conn) => {
      ...
      if (!freshRow) {
        throw new Error("Feature visibility row missing after update");
      }

      const fresh = parseConfigValue(freshRow.config_value);

      if (!fresh) {
        throw new Error("Invalid persisted feature visibility config");
      }
...
async function writePublisherMax(value, adminId) {
  ...
  if (existing) {
    await query(
      "UPDATE app_config SET config_value = ?, updated_by = ?, version = version + 1 WHERE config_key = ?",
      [JSON.stringify(value), uuidToBuffer(adminId), PUBLISHER_MAX_KEY],
    );
  } else {
    await query(
      `INSERT INTO app_config (id, config_key, config_value, is_public, description, version, updated_by) VALUES (?, ?, ?, 1, ?, 1, ?)`,
      [
        uuidToBuffer(generateUuid()),
        PUBLISHER_MAX_KEY,
        JSON.stringify(value),
        "Max verified accounts a publisher may select per post request (1..10)",
        uuidToBuffer(adminId),
      ],
    );
  }
...
export async function getPublisherDeadline(req, res, next) {
  try {
    const hoursRow = await queryOne(
      "SELECT config_value FROM app_config WHERE config_key = ?",
      ["publisher_response_deadline_hours"],
    );
    let hours = 48;
    if (hoursRow) {
      try {
      ...
      const daysRow = await queryOne(
        "SELECT config_value FROM app_config WHERE config_key = ?",
        ["publisher_response_deadline_days"],
      );
      if (daysRow) {
        try {
    ...
    if (existing) {
      ...
      await query(
        ...
        [
          ...
          "publisher_response_deadline_hours",
          JSON.stringify(Math.floor(n)),
          "General waiting time for publishers to accept (hours, 1..720)",
          uuidToBuffer(req.user.id),
        ],
      );
    }
